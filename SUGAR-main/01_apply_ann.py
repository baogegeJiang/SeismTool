...
if __name__ == '__main__':
    ...
    parser.add_argument('--quakeFile', default='../CEA_quakesAll', help='Batch size.', type=str)
    ...
    quakeFile  = args.quakeFile
    quakes = seism.QuakeL(quakeFile)
    mlD={}
    for q in quakes:
        mlD[q.name('_')]=q['ml']
    # ---------------------------------------------------------
    ...
    if os.path.exists(output_dir+'/predicted_prob.txt'):
        with open(output_dir+'/predicted_prob.txt') as f:
            lines = f.readlines()
        index= file_list.index(lines[-1].split(',')[0])
        file_list=file_list[index+1:]
                ...
                if len(raw_data) > 0:
                    ...
                    if sample_window_length > 10:
                        ...
                        if len(surface_wave_data) > surface_window_length*0.99 and len(before_data) > surface_window_length*0.5:
                            ...
                            #
                            features_merged['mag'] = mlD[afile.split('/')[-2]]#float(meta['mag'])
                            #print(features_merged['mag'])
        ...
        if len(x_list) > 0:
            try:
                ...
                for i in range(len(feature_list)):
                    ...
                    if y_deploy[i][0]>0.5:
                        countRight+=1
                        print(COUNT,countRight,len(file_list))
