...
class ODELSTMR(tf.keras.layers.Layer):
    def __init__(self, units, **kwargs):
        self.units = units
        self.state_size = (units, units)
        self.initializer = "glorot_uniform"
        self.recurrent_initializer = "orthogonal"
        self.ctrnn = CTRNNCell(self.units, num_unfolds=4, method="euler")
        super(ODELSTMR, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        if isinstance(input_shape[0], tuple):
            # Nested tuple
            input_dim = input_shape[0][-1]
        self.ctrnn.build(input_shape)
        self.lstm_kernel = self.add_weight(
            shape=(input_dim,4 * self.units),
            initializer=self.initializer,
            name="input_kernel",
        )
    ...
    def call(self, inputs, states,*args, **kwargs):
        ...
        cell_input, new_ode_state = self.ctrnn.call([inputs, elapsed], [ode_state])
        new_ode_state=new_ode_state[0]
        z = (
            tf.matmul(inputs, self.lstm_kernel)
            + tf.matmul(new_ode_state, self.recurrent_kernel)
            + self.bias
        )
        i, ig, fg, og = tf.split(z, 4, axis=-1)
        ...
        new_cell = cell_state * forget_gate + input_activation * input_gate
        cell_output = tf.nn.tanh(new_cell) * output_gate
        # Implementation choice on how to parametrize ODE component
        #ode_output, new_ode_state = self.ctrnn.call([ode_input, elapsed], [ode_state])
        # ode_output, new_ode_state = self.ctrnn.call([ode_input, elapsed], [ode_input])

        return cell_output, [new_cell, new_ode_state]

class ODELSTMR_Old(tf.keras.layers.Layer):
    def __init__(self, units, **kwargs):
        self.units = units
        self.state_size = (units, units)
        self.initializer = "glorot_uniform"
    ...
    def call(self, inputs, states):
        ...
        new_ode_state=new_ode_state[0]
        z = (
            tf.matmul(cell_input, self.lstm_kernel)
            + tf.matmul(ode_state, self.recurrent_kernel)
            + self.bias
        )
        i, ig, fg, og = tf.split(z, 4, axis=-1)
