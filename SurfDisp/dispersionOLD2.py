...
class config:
    ...
    def quakeCorr(self,quakes,stations,byRecord=True,remove_resp=False,para={},minSNR=-1,
        isLoadFv=False,fvD={},isByQuake=False,quakesRef=[],resDir = 'eventSac/',maxCount=-1,**kwags):
        corrL = []
        ...
        for quake in quakes:
            ...
            corrL += corrSacsL(disp,sacsL,sacNamesL,modelFile=self.originName,\
                ...
                isByQuake=isByQuake,maxCount=maxCount,**kwags)
            print('###########',len(corrL))
        return corrL
...
class model:
    ...
    def __init__(self,modelFile, mode='PSV',getMode = 'norm',layerMode ='prem',layerN=10000,isFlat=False,R=6371,flatM=-2,\
        ...
        elif layerMode in '123456':
            index = int(layerMode)
            layerL[0] = layer(0.5*1.7, 0.5, 0.0001,[-100,0])
            for i in range(1,layerN):
                #100.0        7.95      4.45      3.38      200.0      80.0
                #0            1         2         3         4          5
                #vp,vs,rho,z=[0,0],Qp=1200,Qs=600
                ...
                Qp *= QMul
                Qs *= QMul
                #z =np.array([data[i-1,0],data[i,0]])
                z =np.array([data[i-1,0],data[min(i+1-1,layerN-2),0]])
                if isFlat and doFlat:
                    z,vp,vs,rho = flat(z,vp,vs,rho,m=flatM,R=R)
                    print(vp,vs)
    ...
    def __call__(self,f,dist0=0, dist1=0,threshold=0.08,N=1000,randA=0.1):
        ...
        if self.mode == 'fileP':
            ...
            if len(self.f)>2:
               ...
               return f.reshape(shape0)*0+1e-8
...
def outputFvDist(fvD,stations,t=(16**np.arange(0,1.000001,1/49))*10,keys=[],keysL=[]):
    ...
    for key in keysL:
        ...
        if len(keys)!=0:
            if name  not in keys and name1  not in keys:
                continue 
...
def compareFVD(fvD,fvDGet,stations,filename,t=(12**np.arange(0,1.000001,1/49))*10,keys=[],fStrike=1,title='err0_dis',threshold=0.015,delta=1,thresholdForGet=0,isCount=False,plotRate=0.6,fvRef={}):
    ...
    plt.hist(FL[(VL>1)*(VLGet>1)*(np.abs(dvR)<=THRESHOLD)],bins=binF)#,norm=colors.LogNorm()
    ...
    if isCount:
        ...
        for i in range(len(fL)):
            ...
            plt.colorbar()
            plt.subplot(2,3,2)
            plt.plot([1/4,1/4],[0,1],'r',linewidth=0.5)
            ...
            for j in range(len(stations)):
                for k in range(j,len(stations)):
                    ...
                    if perHit[i,j,k]<plotRate:
                        ...
                        #plt.text(stations[j]['lo'],stations[j]['la'],stations[j]['sensorName'])
                        #plt.text(stations[k]['lo'],stations[k]['la'],stations[k]['sensorName'])
            plt.subplot(2,3,4)
...
class corr:
    ...
    def __init__(self,xx=np.arange(0,dtype=np.complex),timeL=np.arange(0),dDis=0,fs=0,\
        ...
        x1=np.arange(0),quakeName='',maxCount=-1,up=1):
        self.maxCount = -1
    ...
    def compareInOut(self,yin,yout,t0,threshold=0.5):
        ...
        posIn   = yin.argmax(axis=0)*delta+t0+1e-8
        posOut  = yout.argmax(axis=0)*delta+t0+1e-8
        dPos    = posOut - posIn
        dPosR   = dPos/posIn
    ...
    def show(self,d,FV):
        #to show the correlation and its arrival time for each frequencies
        linewidth=0.3
    ...
    #this part is for io, not necessary in the lastest version
    def output(self):
...
class corrL(list):
    ...
    def getTimeDis(self,iL,fvD={},T=[],sigma=2,maxCount=512,noiseMul=0,byT=False,\
        ...
        modelNameO='',noY=False,randMove=False,randA=0.03,midV=4,randR=0.5,mul=1):
        #print('sigma',sigma)
        if len(iL)==0:
    ...
    def getTimeDisNew(self,iL,fvD={},T=[],sigma=2,maxCount=512,noiseMul=0,byT=False,\
        byA=False,rThreshold=0.1,byAverage=False,set2One=False,move2Int=False,\
        modelNameO='',noY=False,randMove=False,up=1):
        if len(iL)==0:
            iL=np.arange(len(self))
        if not isinstance(iL,np.ndarray):
            iL = np.array(iL).astype(np.int)
