...
cdef int jsol
cdef int index
cdef double a[200],b[200],d[200],rho[200],X[20],Y[20]
...
cpdef double[:] group(double[:] D, double[:]A,double[:] B,double[:] Rho,double[:]T, int mode=1,bint isR=True, double sone0=1.5, double dc0=0.001,bint isFlat=False, double ar=6370,double domega = 0.001,double dh=0.01):
    ...
    cdef double c0,omega,wvno,det0,c1,omega1,ddc
    cdef double[:] vg ,c
    ...
    for i in xrange(TN):
        ...
        wvno0 =  omega0/c0
        if isR:
            det0 = dltarR0(d,a,b,rho,wvno0,omega0,LN)
        else:
            det0 = dltarR0(d,a,b,rho,wvno0,omega0,LN)
        c1 = c[i]+dc0
        omega1 = twopi/T[i]
        wvno1=  omega1/c1
        if isR:
            ddc = -(dltarR0(d,a,b,rho,wvno1,omega1,LN)-det0)/dc0
        else:
            ddc = -(dltarL0(d,a,b,rho,wvno1,omega1,LN)-det0)/dc0
        c1 = c[i]
        omega1 = twopi/T[i]+domega*omega0
        wvno1=  omega1/c1
        if isR:
            vg[i] = c0/(1- (dltarR0(d,a,b,rho,wvno1,omega1,LN)-det0)/domega/ddc/c0)
        else:
            vg[i] = c0/(1-(dltarL0(d,a,b,rho,wvno1,omega1,LN)-det0)/domega/ddc/c0)
    return vg
cpdef double[:,:,:] kernel(double[:] D, double[:]A,double[:] B,double[:] Rho,double[:]T, int mode=1,bint isR=True, double sone0=1.5, double dc0=0.001,bint isFlat=False, double ar=6370,double domega = 0.001,double dh=0.01):
    #print(A[0],B[0])
    cdef int LN,TN
    LN = len(A)
    TN = len(T)
    ...
    cdef double[:]c
    c=np.zeros(TN)
    K=np.zeros([5,TN,LN], dtype=np.float64)
    CalDisp(D, A, B, Rho,T,c,LN, TN , mode,isR, sone0, dc0,isFlat, ar)
    for i in xrange(TN):
        c0 = c[i]
        omega0 = twopi/T[i]
        ...
        wvno1=  omega1/c1
        if isR:
            K[4,i,0] = c0/(1- (dltarR0(d,a,b,rho,wvno1,omega1,LN)-det0)/domega/ddc/c0)
        else:
            K[4,i,0] = c0/(1-(dltarL0(d,a,b,rho,wvno1,omega1,LN)-det0)/domega/ddc/c0)
        for j in xrange(LN):
            if isFlat:
                A[j]=A[j]+dc0
                ...
                a[j]=a[j]+dc0
            if isR:
                K[0,i,j] = (dltarR0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            else:
                K[0,i,j] = (dltarL0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            if isFlat:
                A[j]=A[j]-dc0
            else:
                a[j]=a[j]-dc0
        for j in xrange(LN):
            if isFlat:
                ...
                B[j]=B[j]+dc0
            if isR:
                K[1,i,j] = (dltarR0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            else:
                K[1,i,j] = (dltarL0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            if isFlat:
                B[j]=B[j]-dc0
            else:
                b[j]=b[j]-dc0
        for j in xrange(LN):
            if isFlat:
                ...
                rho[j]=rho[j]+dc0
            if isR:
                K[2,i,j] = (dltarR0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            else:
                K[2,i,j] = (dltarL0(d,a,b,rho,wvno0,omega0,LN)-det0)/dc0/ddc
            if isFlat:
                Rho[j]=Rho[j]-dc0
            else:
                rho[j]=rho[j]-dc0
        for j in xrange(LN):
            if isFlat:
                ...
                d[j]=d[j]+dh
            if isR:
                K[3,i,j] = (dltarR0(d,a,b,rho,wvno0,omega0,LN)-det0)/dh/ddc
            else:
                K[3,i,j] = (dltarL0(d,a,b,rho,wvno0,omega0,LN)-det0)/dh/ddc
            if isFlat:
                D[j]=D[j]-dh
            else:
                d[j]=d[j]-dh
...
cdef double dltarL0(double[:]d,double[:]a,double[:]b,double[:]rho,double wvno,double omega,int LN):
    '''
    c   find SH dispersion values.
    c
        parameter (NL=100,NP=60)
    ...
    return e1
#@jit
cdef double dltarR0(double[:]d,double[:]a,double[:]b,double[:]rho,double wvno,double omega,int LN):
    #print(LN)
    #cdef double a0, w,cosp,a0,cpcq,cpy,cpz,cqw,cqx,xy,xz,wy,wz
    global E0,E1,E2,E3,E4
    global E01,E11,E21,E31,E41,cosp,w0
