...
class run:
	...
	def plotStaDis(self,isAll=False,isAllQuake=False,resDir=''):
		para = self.config.para
		if isAllQuake:
			stations = seism.StationList(self.config.para['stationFileL'][0])
		else:
		...
		if not isAll:
			...
			if R[0]<25:
				nc='data/etopo05.nc'#'/media/jiangyr/MSSD/ETOPO1_Ice_g_gmt4_new.grd'#'data/etopo05.nc'
			else:
				nc='/media/jiangyr/MSSD/ETOPO1_Ice_g_gmt4NE.grd'
			pc=mt.plotTopo(m,R,topo=nc,isColorbar=False,cpt=mt.cmapETopo,vmin=-10000,vmax=10000)
			haveOne=False
			for fault in mt.faultL:
				if fault.inR(R):
					if haveOne:
						fault.plot(m,linewidth=0.5,)
					else:
						fault.plot(m,linewidth=0.5,label='fault')
						haveOne=True
			...
			if len(stations)>100:
				...
				markersize=4
			m.plot(stx,sty,'^r',label='station',markersize=markersize)
	...
	def loadCorr(self,isLoad=True,isLoadFromMat=False,isGetAverage=True,isDisQC=False,isAll=True,isSave=False,isAllTrain=True,isControl=False,isHalf=False,isBad=False,isSyn=False,**kwags):
		...
		for i in range(N):
			...
			if 'time0' in self.config.para:
				q.select({'time0':self.config.para['time0']})
			quakes  += q
			dRatioD={}
			if 'time0' in self.config.para:
				fvd, q0 = para['dConfig'].loadQuakeNEFV(sta,quakeFvDir=para['pairDirL'][i],quakeD=quakes,time0=self.config.para['time0'],dRatioD=dRatioD,**kwags)
			else:
			...
			if isBad:
				for key in self.badL:
					if key in fvd:
						fvd.pop(key)
						badCount+=1
				print('#####################badCount',badCount,'##',len(self.badL))
			if isLoad:
				if not isLoadFromMat:
					corrL0  = para['dConfig'].quakeCorr(q,sta,\
							...
							fvD=fvD,isByQuake=para['isByQuakeL'][i],para=para['sacPara'],resDir=para['eventDir'],maxCount=para['maxCount'],up=para['up'],isIqual=para['isIqual'])
					corrL   += corrL0
				else:
					corrL = d.corrL()
					if isAll:
			...
			if isGetAverage:
				...
				fvDA = d.fvM2Av(fvMGet,threshold=para['qcThreshold'],minThreshold=para['minThreshold'],minSta=para['minSta'],it=para['trainIt'])
				#fvMGet  =d.fvD2fvM(fvd,isDouble=True)
				if isHalf:
					fvDAHalf = d.fvM2Av(fvMGet,threshold=para['qcThreshold'],minThreshold=para['minThreshold'],minSta=para['minSta']*isHalf,it=para['it'])
			else:
			...
			for key in fvDA:
				...
				if isHalf and key in fvDAHalf:
					fvDAHalfO[key]=fvDAHalf[key].copy()

			if isDisQC:
				d.disQC(fvDA,stations,fvDAverage['models/prem'],randA=disRandA,maxR=disMaxR)

			d.qcFvD(fvDA,threshold=para['thresholdTrain'],delta=delta,stations= sta,minCount=para['minCount'])
			d.qcFvD(fvDAO,threshold=para['thresholdTrain'],delta=delta,stations= sta,minCount=para['minCount'])
			fvDAverage.update(fvDA)
			fvDAverageO.update(fvDAO)
			if isHalf:
				d.qcFvD(fvDAHalf,threshold=para['thresholdTrain'],delta=delta,stations= sta,minCount=para['minCount'])
				d.qcFvD(fvDAHalfO,threshold=para['thresholdTrain'],delta=delta,stations= sta,minCount=para['minCount'])
				fvDAverageHalf.update(fvDAHalf)
				fvDAverageHalfO.update(fvDAHalfO)
				...
				if len(key.split('_'))>=2:
					...
					if modelName0 in fvDAverageNew or modelName1 in fvDAverageNew:
						if key in fvD0 or isAllTrain:
							corrL1.append(corr)
							if isSyn and np.random.rand()<0.15:
								corrL1.append(corr.copy())
								corrL1[-1].dDis = corrL1[-1].dDis*(1+(2*np.random.rand()-1)*0.1)
		...
		self.fvDAverageO ={'models/prem':fvDAverage['models/prem']}
		self.fvDAverageHalf ={'models/prem':fvDAverage['models/prem']}
		self.fvDAverageHalfO ={'models/prem':fvDAverage['models/prem']}
		for key in fvDAverage:
			self.fvDAverageO[key]=fvDAverageO[key]
			if isHalf:
				if key in fvDAverageHalf:
					self.fvDAverageHalf[key]=fvDAverageHalf[key]
				if key in fvDAverageHalfO:
					self.fvDAverageHalfO[key]=fvDAverageHalfO[key]
		self.fvD    = fvDNew
		self.fvD0   = fvD0New
		self.fvDO =fvDO
		self.quakes = quakes
		self.stations = stations
		self.fvDAverageNewCount=fvDAverageNewCount
		self.fvDAverageNewAllCount=fvDAverageNewAllCount
		if isSave:
	...
	def train(self,isRand=True,isShuffle=False,isAverage=False):
		...
		set2One=para['set2One'],move2Int=para['move2Int'],randMove=True,randA=para['randA'],midV=para['midV'],disAmp=para['disAmp'],fromT=para['fromT'])
		...
		fcn.trainAndTest(self.model,self.corrLTrain,self.corrLValid,self.corrLTest,\
                        outputDir=para['trainDir'],sigmaL=[sigma],tTrain=tTrain,perN=2048,count0=3,w0=1,k0=1e-2)#w0=3#4
		#fcn.trainAndTest(self.model,self.corrLTrain,self.corrLValid,self.corrLTest,\
	...
	def trainMul(self,isRand=True,isShuffle=False,isAverage=False,isRun=True,isAll=False,**kwargs):
		...
		byT=False,noiseMul=0.0,byA=False,rThreshold=1e-2,byAverage=False,\
		...
		if isRun:
			self.config.para['modelFile']=fcn.trainAndTestMul(self.model,self.corrDTrain,self.corrDValid,self.corrDTest,\
							outputDir=para['trainDir'],sigmaL=[sigma],tTrain=tTrain,perN=2048,count0=3,w0=1,k0=1e-2,mul=para['mul'])#w0=3#4
	...
	def trainSq(self):
		...
		set2One=para['set2One'],move2Int=para['move2Int'],randMove=True,disAmp=para['disAmp'],fromT=para['fromT'])
	...
	def trainDt(self):
		...
		tTrain = para['T']
		self.corrLTrain.setTimeDis(self.fvD,tTrain,sigma=1.5,maxCount=para['maxCount'],\
		byT=True,noiseMul=0.0,byA=False,rThreshold=1e-6,byAverage=False,\
		set2One=para['set2One'],move2Int=para['move2Int'],randMove=True,disAmp=para['disAmp'],fromT=para['fromT'],fvDSyn=self.fvDSyn)
		self.corrLTest.setTimeDis(self.fvD,tTrain,sigma=1.5,maxCount=para['maxCount'],\
		byT=True,noiseMul=0.0,byA=False,rThreshold=1e-6,byAverage=False,\
		set2One=para['set2One'],move2Int=para['move2Int'],randMove=True,disAmp=para['disAmp'],fromT=para['fromT'],fvDSyn=self.fvDSyn)
		self.corrLValid.setTimeDis(self.fvD,tTrain,sigma=1.5,maxCount=para['maxCount'],\
	...
	def calResOneByOne(self,isLoadModel=False,isPlot=False,isRand=False):
		...
		for i in range(N):
			...
			for j in range(self.config.para['gpuIndex'],int(len(q)/perN)+1,self.config.para['gpuN']):#self.config.para['gpuIndex']
				...
				corrL0  = para['dConfig'].quakeCorr(q[j*perN:min(len(q)-1,j*perN+perN)],sta,byRecord=para['byRecordL'][i],remove_resp=para['remove_respL'][i],minSNR=para['minSNRL'][i],isLoadFv=False,fvD=fvDAverage,isByQuake=para['isByQuakeL'][i],para=para['sacPara'],resDir=para['eventDir'],maxCount=para['maxCount'],plotDir=plotDir,isIqual=para['isIqual'])
				...
				self.corrL.setTimeDis(fvDAverage,para['T'],sigma=1.5,maxCount=para['maxCount'],\
							...
							set2One=para['set2One'],move2Int=para['move2Int'],modelNameO=para['refModel'],noY=True,randMove=isRand,randA=para['randA'],disAmp=para['disAmp'],fromT=para['fromT'])
			...
			for i in range(N):
				...
				for j in range(self.config.para['gpuIndex'],int(len(q)/perN)+1,self.config.para['gpuN']):#self.config.para['gpuIndex']
					...
					corrL0  = para['dConfig'].quakeCorr(q[j*perN:min(len(q)-1,j*perN+perN)],sta,\
							...
							resDir=para['eventDir'],maxCount=para['maxCount'],plotDir=plotDir,isIqual=para['isIqual'])
					self.corrL  = d.corrL(corrL0,maxCount=para['maxCount'])
					if len(self.corrL)==0:
						continue
					...
					self.corrL = 0
					gc.collect()
	def calFromCorr(self,isLoadModel=False,M=2000,isRand=False):
		config     = self.config
		para       = config.para
		N          = len(para['stationFileL'])
		matDir     = para['matDir']
			...
			with h5py.File(para['matH5']) as h5:
				...
				for j in range(len(sta)):
					...
					for k in range(j,len(sta)):
						...
						corrL.loadByPairsH5([sta0+'_'+sta1],h5)
						nowTime = time()
						perTime = (nowTime-lastTime)/(len(corrL)+0.00000000000000001)
						useTime=nowTime-sTime
						doneN = j*(len(sta)+len(sta)-j)/2+k-j+0.000000000000001
						remainN = len(sta)*len(sta)/2-doneN
						remainTime = useTime/doneN*remainN
						print('%d of %d: %d perTime: %.3f ms predict cost: %.3f h process: %.3f h || %.3f h'%(j,len(sta),k,perTime*1000,predictCost/3600,useTime/3600,remainTime/3600))
						#print(j,'of',len(sta),k,'perTime:',perTime,'s','process:',useTime/3600,'|',remainTime/3600)
						if len(corrL)>M or (j==len(sta)-2 and k==j+1 and len(corrL)>0 ):
							...
							eTimePredict=time()
							print('predicted')
							predictCost+=eTimePredict-sTimePredict
							lastTime=time()
							corrD.corrL=0
							corrD=0
							corrL = d.corrL()
	...
	def rePick(self):
		...
		self.fvDGet = fvdRe
		run.d.saveFVD(fvdRe,self.stations,self.q,'%s/CEA_P_'%para['resDir'],'pair',isOverwrite=True)
	def calRes(self):
		para = self.config.para
		self.corrL1.setTimeDis(self.fvDAverage,para['T'],sigma=1.5,maxCount=para['maxCount'],\
		byT=False,noiseMul=0.0,byA=False,rThreshold=0.0,byAverage=True,\
		set2One=para['set2One'],move2Int=para['move2Int'],modelNameO=para['refModel'],noY=True,disAmp=para['disAmp'],fromT=para['fromT'])
	...
	def getAv(self,isMinP=True,isDisQC=False,isCoverQC=False,delta=1,isWeight=False,weightType='std',isHalf=False):
		if isMinP:
		...
		if isHalf!=False:
			for key in self.fvMGet:
				self.fvMGet[key] = self.fvMGet[key][::isHalf]
	...
	def analyRes(self,threshold=0.015,format='eps',isAverage=False,isHalf=False,onlySingle=False):
		resDir = 'predict/'+self.config.para['resDir'].split('/')[-2]+'/'
		...
		fvTrain = self.fvTrain
		fvTest = self.fvTest
		fvValid = self.fvValid
		fvDAverage = self.fvDAverage
		if isHalf:
			fvDAverage = self.fvDAverageHalf
			fvTrain = []# self.fvTrain
			fvTest = []#self.fvTest
			fvValid = []#self.fvValid
			for key in self.fvTrain:
				if self.fvDAverageNewCount[key]>=self.config.para['minSta']*isHalf and key in fvDAverage:
					fvTrain.append(key)
			for key in self.fvTest:
				if self.fvDAverageNewCount[key]>=self.config.para['minSta']*isHalf and key in fvDAverage:
					fvTest.append(key)
			for key in self.fvValid:
				if self.fvDAverageNewCount[key]>=self.config.para['minSta']*isHalf and key in fvDAverage:
					fvValid.append(key)

		fStrike=2
		if not onlySingle:
			averageTrain=d.compareFVD(fvDAverage,self.fvAvGet,self.stations,resDir+'erroTrain_%.3f_%.3f.%s'%(threshold,self.config.para['threshold'],format),t=self.config.para['T'],keys=fvTrain,fStrike=fStrike,title='E. D. on Train',threshold=threshold,thresholdForGet=self.config.para['threshold'])
		singleTrain=d.compareFVD(fvD,self.fvDGet,self.stations,resDir+'erroTrainSingle_%.3f_%.3f.%s'%(threshold,self.config.para['minP'],format),t=self.config.para['T'],keys=fvTrain,fStrike=fStrike,title='E. D. S. on Train',threshold=threshold,thresholdForGet=-self.config.para['minP'],isCount=False,plotRate=0.6,fvRef=self.fvDAverage['models/prem'])
		if not onlySingle:
			averageValid=d.compareFVD(fvDAverage,self.fvAvGet,self.stations,resDir+'erroValid_%.3f_%.3f.%s'%(threshold,self.config.para['threshold'],format),t=self.config.para['T'],keys=fvValid,fStrike=fStrike,title='E. D. on Valid',threshold=threshold,thresholdForGet=self.config.para['threshold'])
		singleValid=d.compareFVD(fvD,self.fvDGet,self.stations,resDir+'erroValidSingle_%.3f_%.3f.%s'%(threshold,self.config.para['minP'],format),t=self.config.para['T'],keys=fvValid,fStrike=fStrike,title='E. D. S. on Valid',threshold=threshold,thresholdForGet=-self.config.para['minP'],isCount=False,plotRate=0.6,fvRef=self.fvDAverage['models/prem'])
		if not onlySingle:
			averageTest=d.compareFVD(fvDAverage,self.fvAvGet,self.stations,resDir+'erroTest_%.3f_%.3f.%s'%(threshold,self.config.para['threshold'],format),t=self.config.para['T'],keys=fvTest,fStrike=fStrike,title='E. D. on test',threshold=threshold,thresholdForGet=self.config.para['threshold'])
		singleTest=d.compareFVD(fvD,self.fvDGet,self.stations,resDir+'erroTestSingle_%.3f_%.3f.%s'%(threshold,self.config.para['minP'],format),t=self.config.para['T'],keys=fvTest,fStrike=fStrike,title='E. D. S. on test',threshold=threshold,thresholdForGet=-self.config.para['minP'])
		with open(resDir+'resOnTrainTestValid_%.3f'%threshold,'w') as f:
			...
			if not onlySingle:
				...
				f.write('%s&single&%s\\\\[2pt]\n'%('valid',singleValid))
				f.write('%s&single&%s\\\\[2pt]\n'%('test',singleTest))
		with open(resDir+'validPair','w') as f:
			f.write('%d %d %d'%(len(fvTrain),len(fvValid),len(fvTest)))
		...
		if not onlySingle:
			fvMRef  =d.fvD2fvM(fvD,isDouble=True)
			d.plotFVM(self.fvMGet,self.fvAvGet,fvDAverage,resDir=resDir+'/compare/',isDouble=True,fL0=1/self.config.para['T'],format=format,stations=self.stations,keyL=fvTest,threshold=threshold,fvMRef=fvMRef)
	def getAV(self):
	...
	def areasLimit(self):
		...
		d.qcFvD(self.fvAvGet)
	def preDS(self,do=True,isByTrain=False,threshold=-1,**kwargs):
		self.config.para['runDir']='predict/'+self.config.para['resDir'].split('/')[-2]+'/'+'DS/'
		para    = self.config.para
		tSur = para['tSur']
		...
		if do:
			...
			self.indexL = indexL
			self.vL   = vL
			DS.test(vL,indexL,self.stations,**kwargs)
	...
	def preDSTrain(self,do=True,**kwargs):
		para    = self.config.para
		tSur = para['tSur']
		z= para['z'];surPara= para['surPara'];DSConfig = DSur.config(para=surPara,z=z)
		...
		if do:
			...
			self.vL   = vL
			DS.test(vL,indexL,self.stations,**kwargs)
	def loadAndPlot(self,DS=[],isPlot=True,**kwargs):
		if isinstance(DS,list):
			DS = self.DS
		DS.loadRes()
		if isPlot:
			DS.plotByZ(p2L=self.config.para['p2L'],R=self.config.para['R'],**kwargs)
	...
	def preDSSyn(self,do=True,isByTrain=False,M=1,**kwargs):
		self.config.para['runDir']='predict/'+self.config.para['resDir'].split('/')[-2]+'/'+'DS/'
		para    = self.config.para
		tSur = para['tSur']
		z= para['z']
		...
		if do:
			...
			self.indexL = indexL
			self.vL   = vL
			DS.testSyn(vL,indexL,self.stations[-1::-1],M=1,**kwargs)
	def plotTrainDis(self,isAverage=False,isSyn=False):
		para = self.config.para
		resDir = 'predict/'+self.config.para['resDir'].split('/')[-2]+'/'
		if not os.path.exists(resDir):
			os.makedirs(resDir)
		...
		fvAverageSingle = d.averageFVL([fvD[key] for key in fvD],fL=1/self.config.para['T'])
		thresL=[0.015]
		#return fvAverage
		print(fvAverage.f,fvAverage.f)
		with open(resDir+'dataSetting','w') as f:
			f.write('single setting\n')
			f.write('set curve pick noise syn\n')
			synCount=0
			for corr in self.corrDTrain.corrL:
				if corr.isSyn:
					synCount+=1
			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvD0,self.stations,t=self.config.para['T'],keys=self.fvTrain)
			f.write('train %d %d %d %d\n'%(len(vL),(vL>1).sum(),len(self.corrDTrain.corrL)-len(vL)-synCount,synCount))
			...
			synCount=0
			for corr in self.corrDValid.corrL:
				if corr.isSyn:
					synCount+=1
			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvD0,self.stations,t=self.config.para['T'],keys=self.fvValid)
			f.write('valid %d %d %d %d\n'%(len(vL),(vL>1).sum(),len(self.corrDValid.corrL)-len(vL)-synCount,synCount))
			...
			synCount=0
			for corr in self.corrDTest.corrL:
				if corr.isSyn:
					synCount+=1
			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvD0,self.stations,t=self.config.para['T'],keys=self.fvTest)
			f.write('Test %d %d %d %d\n'%(len(vL),(vL>1).sum(),len(self.corrDTest.corrL)-len(vL)-synCount,synCount))
			...
			d.plotFV(vL,fL,resDir+'FVTrainSingle.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVTrainSingleRand.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)

			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(fvD,self.stations,t=self.config.para['T'],keys=self.fvTest)
			d.plotFV(vL,fL,resDir+'FVTestSingle.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVTestSingleRand.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)

			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(fvD,self.stations,t=self.config.para['T'],keys=self.fvValid)
			d.plotFV(vL,fL,resDir+'FVValidSingle.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVValidSingleRand.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)
			if isSyn:
				disL,vL,fL,fvAverage_ ,keyL= d.outputFvDist(self.fvDSynTrain,self.stations,t=self.config.para['T'],isSyn=True)
				d.plotFV(vL,fL,resDir+'FVTrainSingleSyn.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)

				disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvDSynTest,self.stations,t=self.config.para['T'],isSyn=True)
				d.plotFV(vL,fL,resDir+'FVTestSingleSyn.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
				
				disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvDSynValid,self.stations,t=self.config.para['T'],isSyn=True)
				d.plotFV(vL,fL,resDir+'FVValidSingleSyn.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
	...
	def plotGetDis(self):
		...
		if not os.path.exists(resDir):
			os.makedirs(resDir)
		fvAverage = d.averageFVL([self.fvDAverage[key] for key in self.fvDAverage],fL=1/self.config.para['T'])
		fvAverageSingle = d.averageFVL([self.fvDGet[key] for key in self.fvDGet],fL=1/self.config.para['T'])
		thresL=[0.015]
		#return fvAverage
		print(fvAverage.f,fvAverage.f)
		with open(resDir+'getSettingO','w') as f:
			trainCount=0
			validCount=0
			testCount=0
			for corr in self.corrL:
				netSta0, netSta1 = corr.modelFile.split('_')[-2:]
				key = netSta0+'_'+netSta1
				if key in self.fvTrain:
					trainCount += 1
				if key in self.fvValid:
					validCount += 1
				if key in self.fvTest:
					testCount += 1
		...
		with open(resDir+'getSetting','w') as f:
			...
			disL,vL,fL,fvAverage_ ,keyL= d.outputFvDist(self.fvDGet,self.stations,t=self.config.para['T'],keys=self.fvTrain)
			d.plotFV(vL,fL,resDir+'FVTrainSingleGet.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVTrainSingleRandGet.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)

			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvDGet,self.stations,t=self.config.para['T'],keys=self.fvTest)
			d.plotFV(vL,fL,resDir+'FVTestSingleGet.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVTestSingleRandGet.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)

			disL,vL,fL,fvAverage_,keyL = d.outputFvDist(self.fvDGet,self.stations,t=self.config.para['T'],keys=self.fvValid)
			d.plotFV(vL,fL,resDir+'FVValidSingleGet.eps',isAverage=True,fvAverage=fvAverageSingle,thresL=thresL)
			d.plotFV(vL,fL,resDir+'FVValidSingleRandGet.eps',isAverage=True,fvAverage=fvAverageSingle,isRand=True,randA=para['randA'],midV=para['midV'],randN=2,randR=0.5,thresL=thresL)
	...
	def plotDVK(self,fvD,fvRef,format='jpg',head='',fvD0={},isRight=True,isPlot=True,isRand=False):
		f = 1/self.config.para['T']
		DV,K,V,SNR = d.getDVK(fvD,f,fvD0=fvD0,isRight=isRight)
		if isRand:
			#DV*=1+(np.random.rand(*DV.shape)*2-1)*self.config.para['randA']*np.random.rand(*DV.shape)
			rand = (np.random.rand(*V.shape)*2-1)*self.config.para['randA']*np.random.rand(*V.shape)
			if np.random.rand()>0.5:
				rand=V*rand/self.config.para['midV']
				DV*=1+rand
			V*= 1+rand
		resDir = 'predict/'+self.config.para['resDir'].split('/')[-2]+'/'
		if not os.path.exists(resDir):
		...
		plt.gca().set_position([0.15,0.15,0.7,0.7])
		#if isRight:
		plt.plot(self.minSNR,f,'r',linewidth=0.5,label='$SNR_{0.95}$')
		#plt.plot(KRef,f,'r',linewidth=0.5,label='ref')
	...
	def calByDKV(self,corrL,fvRef,k=0,maxCount=-1,fvD0={},isRand=False,**kwags):
		fvD = {}
		f = 1/self.config.para['T']
		...
		for corr in corrL[:maxCount]:
			...
			if isRand:
				rand = (np.random.rand()*2-1)*self.config.para['randA']*np.random.rand()
				dis0 = corr.dis
				timeL0=corr.timeL
				midV=self.config.para['midV']
				dDis0 = np.abs(dis0[0]-dis0[1])
				dDis = dDis0
				dT = 0
				#timeL0=corr.timeL
				if np.random.rand()>0.5:
					corr.dis=[dis0[0],dis0[1]+dDis0*rand]
					dDis =  np.abs(corr.dis[0]-corr.dis[1])
				else:
					dT=dDis0/midV*rand
					corr.timeL=timeL0+dT
...
paraTrainTest={ 'quakeFileL'  : ['CEA_quakesAll'],\
	...
	'matH5'	  :'/media/jiangyr/1TSSD/trainTest4Hz_20220402_snrSTD3_noX01_V100000.h5',#/media/jiangyr/1TSSD/trainTest2Hz_0_New8_snrSTD2.5V8.h5
	'badFile' : 'data/badFVD',
	'randA'       : 0.04,\
	'disAmp'      : 1,\
	'fromT'       : -384,
	...
	'synDir'      : 'models/syn/',\
	'resDir'      : '/media/jiangyr/MSSD/20220405_it2_0.02_V800000/',#'models/ayu/Pairs_pvt/',#'results/1001/',#'results/1005_allV1/',\
	'perN'        : 20,\
	'eventDir'    : '/media/jiangyr/1TSSD/eventSac/',\
	'z'           :[0,2.5,7.5,10,15,20,25,30,35,40,45,50,60,70,80,100,120,140,180,220,280,360],#[5,10,20,30,45,60,80,100,125,150,175,200,250,300,350](350**(np.arange(0,1.01,1/18)+1/18)).tolist(),\
	...
	'surPara'     : { 'nxyz':[19,28,14], 'lalo':[55,110],#[40,60,0][55,108]\
					...
					'maxIT':15,'nBatch':1,'smoothDV':30,'smoothG':20,'vR':np.array([[53.5,122.1],[48,134.1],[42.1,131.1],[39.1,125],[39.9,115.1],[42,111.9],[45,111.9],[53.5,122.1]]),'perAGs':0.025,'perAGc':0.025,'perN':[3,3,5],'perNG':[6,6,5],'noiselevel':0.00,'perA':0.03,'iso':'F',},\
...
paraAll={ 'quakeFileL'  : ['CEA_quakesAll'],\
	...
	'modelFile'   : '/home/jiangyr//Surface-Wave-Dispersion/SeismTool/predict/0130_0.95_0.05_3.2_randMove/resStr_220314-063855_model.h5',
	'isLoadFvL'   : [True],#False********\
	'byRecordL'   : [True],\
	'maxCount'    : 512*2,\
	'minSNRL'     : [2.5],\
	...
	'matDir'	  :'/media/jiangyr/1TSSD/matDirAll/',\
	'trainH5'	  :'/media/jiangyr/1TSSD/trainSet.h5',\
	'matH5'	  :'/media/jiangyr/1TSSD/allclip20220318STD.h5',\
	'randA'       : 0.05,\
	'disAmp'      : 1,
	'midV'        : 4,\
	'mul'		  : 1,\
	...
	'set2One'	  : True,
	'trainDir'    : 'predict/0130_0.95_0.05_3.2_randMove/',\
	'resDir'      : '/media/jiangyr/MSSD/resStr_220314-063855_all/',#'models/ayu/Pairs_pvt/',#'results/1001/',#'results/1005_allV1/',\
	'perN'        : 10,\
	'eventDir'    : '/media/jiangyr/1TSSD/eventSac/',\
...
[[45,115],[35,115]],
	...
	[[33,105],[50,130]],
'''
paraTibet={ 'quakeFileL'  : ['CEA_quakesAll'],\
	'stationFileL': ['../stations/CEA.sta_know_few'],#**********'stations/CEA.sta_know_few'\
	'modelFile'   : '/home/jiangyr//Surface-Wave-Dispersion/SeismTool/predict/0130_0.95_0.05_3.2_randMove/resStr_220314-063855_model.h5',
	'isLoadFvL'   : [True],#False********\
	'byRecordL'   : [True],\
	...
	'Tav'         : (16**np.arange(0-1/N1,1.000001+1/N1,1/N1))*10,\
	'tSur'        : (16**np.arange(0,1.000001,1/N2))*10,\
	'surPara'     : { 'nxyz':[19,18,16], 'lalo':[36,96],#[40,60,0][55,108]\
					'dlalo':[1,1], 'maxN':150,#[0.5,0.5]\
					'kmaxRc':0,'rcPerid':[],'threshold':0.01,'sparsity': 1,\
					'maxIT':15,'nBatch':2,'smoothDV':5,'smoothG':10,'vR':np.array([[26,99],[34.1,99],[34.1,104],[35,104],[35,109.1],[22,109],[22,102.5],[26,100],[26,99]]),'perAGs':0.0,'perAGc':0.0,'perN':[3,3,4],'perNG':[6,6,4],'noiselevel':0.00,'perA':0.04,'iso':'T'},\
	'runDir'      : '../DS/20220111_CEA160_TrainTest/',#_man/',\
	'gpuIndex'    : 0,\
	'gpuN'        : 1,\
	'lalo'        :[20,35,97,110],#[-90,90,0,180],#[38,54,110,134],#[20,34,96,108][]*******,\
	'minThreshold':0.01,\
	'thresholdTrain'   :0.02,\
	'thresholdTrainDiff'   :0.02,\
	'threshold'   :0.015,\
