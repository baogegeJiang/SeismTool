...
def wLoss(q,q1,q2,a):
	line = np.arange(1,len(q)+1)
	return (1-a)*stats.wasserstein_distance(line[q>0],line[q1>0],q[q>0],q1[q1>0])+a*stats.wasserstein_distance(line[q>0],line[q2>0],q[q>0],q2[q2>0])

def wLoss2(q,q1,q2,a):
	line = np.arange(1,len(q)+1)
	return (1-a)*stats.wasserstein_distance(line,line,q,q1)**2+a*stats.wasserstein_distance(line,line,q,q2)**2

def g(q,q1,q2,a,loss,d=1e-4):
	w = q*0
	L0 = loss(q,q1,q2,a)
	print(L0)
	for i in range(w.shape[0]):
		...
		q    /=q.sum()
	return w,L0
def g2(q,q1,q2,a,loss,d=0.001):
	w = q*0
	L0 = loss(q,q1,q2,a)
	print(L0)
	for i in range(w.shape[0]):
		qTmp = q.copy()
		r = d*np.random.rand(w.shape[0])
		r -= r.mean()
		qTmp +=r
		q[q<=0.001]=0.001
		qTmp/=qTmp.sum()
		if loss(qTmp,q1,q2,a)<L0:
			q=qTmp
	return w,L0
def findW(q1,q2,alpha,loss=wLoss,r=0.1,N=1):
	q=q1*(1-alpha/N)+q2*alpha/N
	q/=q.sum()
	L=[]
	for n in range(1,N+1):
		for i in range(200):
			w ,L0= g2(q,q1,q2,alpha*n/N,loss)
			#print(w)
			#w-=w.mean()
			#q-=w*r
			#q[q<=0.001]=0.001
			#q/=q.sum()
			L0 = loss(q,q1,q2,alpha)
			L.append(L0)
	return q,L

def convert(q1,q2,timeL,erro=0.0001,a=0.5):
	q1[q1<erro]=0
	q2[q2<erro]=0
	q1/=q1.sum()
	q2/=q2.sum()
	...
	for i in range(len(q1)):
		if q1[i]>0:
			Q1.append([q1[i],[timeL[i]]])
		if q2[i]>0:
			Q2.append([q2[i],[timeL[i]]])
	geodesic = Wasserstein_Geodesic(Q1, Q2)
	Q = geodesic.Geodesic_Point(a)
	w = [q[0] for q in Q]
	x=  [q[1][0] for q in Q]
	w= np.array(w)
	x=np.array(x)
...
if __name__=='__main__1':
	...
	plt.plot(q1,'b')
	plt.plot(q2,'k')
	plt.plot(x,w,'.r',markersize=2)
	plt.subplot(2,1,2)
	#plt.plot(np.log(L))
	plt.savefig('testW.jpg',dpi=300)

if __name__=='__main__':
	disL= np.arange(-150,150,50)
	timeL = np.arange(-60,80,0.1)
	mul=200
	dt =0.2
	fN = 1/dt*2
	fmin =1/10
	...
	vs = vp/1.7
	Data = []
	b,a=signal.butter(4,[fmin/fN,fmax/fN],'bandpass')
	for dis in disL:
		data = timeL*0
		tp = dis/vp
		ts = dis/vs
		ts_ = -dis/vs+25
		data+= np.exp(-(timeL-tp)**2/0.25)*0.5
		data+= np.exp(-(timeL-ts)**2/0.25)
		data+= np.exp(-(timeL-ts_)**2/0.25)
		data/=data.sum()
		Data.append(data)
		plt.plot(timeL,data*mul+dis,'k',linewidth=0.3)

	Data1 =[]
	disL1 = (disL[:-1]+disL[1:])/2
	for i in range(len(disL1)):
		w,x = convert(Data[i],Data[i+1],timeL,a=0.5)
		data1 = interpolate.interp1d(x,w,bounds_error=False,fill_value=0)(timeL)
		data1/=data1.sum()
		plt.plot(timeL,data1*mul+disL1[i],'r',linewidth=0.3)
		#print(len(x),len(w*mul+disL1[i]))
		#plt.plot(x,np.array(w)*mul+disL1[i],'-r',linewidth=0.3,markersize=0.3)
		#Data1.append(data1)
	#q,L= findW(q1,q2,0.5)
	#plt.plot(np.log(L))
	plt.savefig('testW.jpg',dpi=300)
