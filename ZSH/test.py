...
file=glob('Shot_sgy/Shot_*.sgy')[0]
...
for sourceId in sD:
    ...
    p = g.pair(maxDis=3.5,minDis=0.2,minDDis=0.5,maxDDis=1.5,maxTheta=5,deltaM=0.1,deltaD=0.1,minSnr=3)
    p.plot()
...
for i in range(len(tL)):
    ...
    plt.xlim([XMIN,XMAX])
    plt.ylim([YMIN,YMAX])
    plt.savefig('%s/phaseVel_%.3f(Hz).jpg'%(resDir,1/tL[i]),dpi=300)
    plt.close()
plt.pcolor       
...
if False:
    mul = 4
    dirL = ['/autofs/2022pai/2022_test_code/2022Pai30/02_SuFaceWave_Supression/2021HJQ3D/INPUT/Deonise_B4_Sgy/','/autofs/2022pai/2022_test_code/2022Pai30/02_SuFaceWave_Supression/INPUT/2022PGD/Shot_sgy/']
    #dirL = ['/home/jiangyr/']
    for Dir in dirL:
        newDir = Dir.split('/')[-2]+'/'
        if not os.path.exists(newDir):
            os.makedirs(newDir)
        for file in glob(Dir+'*sgy'):
            st = read(file,unpack_trace_headers=True)
